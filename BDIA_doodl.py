...
class SteppingLayer(nn.Module):
    ...
    def forward(self, i, t, latent_pair,
               reverse=False):
        """
        Run an EDICT step,  refer to edcit paper
        """
        epsilons = []
        for base_latent_i in range(2): 
            ...
            alpha_prod_i, beta_prod_i = get_alpha_and_beta(timestep, self.scheduler)
            alpha_prod_i_prev, _ = get_alpha_and_beta(prev_timestep, self.scheduler)

            alpha_i_1 = alpha_prod_i_prev**0.5 
            alpha_i = alpha_prod_i**0.5

            ## Variable collection for BDIA ##
            sigma_i_1  =  ((1 - alpha_prod_i_prev)**0.5) 
            sigma_i = ((1 - alpha_prod_i)**0.5)#(beta_prod_i ** 0.5)

            alphas = [alpha_i_1, alpha_i]
            sigmas = [sigma_i_1, sigma_i]

            a = np.array(alphas)
            s = np.array(sigmas)

            alphas = torch.tensor(a, requires_grad=True)
            sigmas = torch.tensor(s, requires_grad=True)
            ########                        ########

            epsilons.append(noise_pred)
        ...
        epsilons1 = torch.cat([epsilons[0], epsilons[1]])
        
        #more variables returned for BDIA (epsilons1, alphas, sigmas)
        return i.clone(), t.clone(), combined_outputs, epsilons1, alphas, sigmas
...
class MixingLayer(nn.Module):
    """
    This does the mixing layer of BDIA
    https://arxiv.org/pdf/2307.10829.pdf
    Equations 51/52
    """
    
    def __init__(self, mix_weight=0.93):
        super(MixingLayer, self).__init__()
        self.p = mix_weight     #gamma?
        self.epsilon_y_i_1 = 0  #for BIDA-DOODL
        self.firstdone = False  #for BDIA-DOODL
        self.z_i_t = 0          #for BIDA-DOODL
        self.y_i_t = 0          #for BIDA-DOODL
    
    def forward(self, input_x, epsilons, alphas, sigmas):
        epsilon_z_i, epsilon_y_i = epsilons[0], epsilons[1]

        alpha_i_1, alpha_i = alphas[0], alphas[1]
        sigma_i_1, sigma_i = sigmas[0], sigmas[1]

        z_i, y_i = input_x[:1], input_x[1:]

        if self.firstdone:
            y_i_1 = self.y_i_t + (alpha_i_1 * ((y_i - sigma_i*epsilon_y_i)/alpha_i)) + (sigma_i_1*epsilon_y_i) - y_i
            z_i_1 = self.z_i_t + (alpha_i_1 * ((y_i_1 - sigma_i*self.epsilon_y_i_1)/alpha_i)) + (sigma_i_1*self.epsilon_y_i_1) - y_i_1
        else:
            y_i_1 = z_i + (alpha_i_1 * ((y_i - sigma_i*epsilon_y_i)/alpha_i)) + (sigma_i*epsilon_y_i) - y_i
            z_i_1 = y_i + (alpha_i_1 * ((y_i_1 - sigma_i*epsilon_y_i)/alpha_i)) + (sigma_i_1*epsilon_y_i) - y_i_1
        
        self.firstdone = True
        self.epsilon_y_i_1 = epsilon_y_i
        self.z_i_t = z_i
        self.y_i_t = y_i

        return torch.cat([z_i_1, y_i_1])

    def inverse(self, input_x, epsilons, alphas, sigmas):
        '''This equation needs checking and fixing'''
        epsilon_z_i, epsilon_y_i = epsilons[0], epsilons[1]

        alpha_i_1, alpha_i = alphas[0], alphas[1]
        sigma_i_1, sigma_i = sigmas[0], sigmas[1]

        y_i, z_i = input_x[:1], input_x[1:]
        
        #TODO# check in implement correctly
        if self.firstdone:
            y_i_1 = self.y_i_t + (alpha_i_1 * ((y_i - sigma_i*self.epsilon_y_i_1)/alpha_i)) + (sigma_i_1*self.epsilon_y_i_1) - y_i
            z_i_1 = self.z_i_t + (alpha_i_1 * ((y_i_1 - sigma_i*epsilon_y_i)/alpha_i)) + (sigma_i_1*epsilon_y_i) - y_i_1
        else:
            y_i_1 = z_i + (alpha_i_1 * ((y_i - sigma_i*epsilon_y_i)/alpha_i)) + (sigma_i*epsilon_y_i) - y_i
            z_i_1 = y_i + (alpha_i_1 * ((y_i_1 - sigma_i*epsilon_y_i)/alpha_i)) + (sigma_i_1*epsilon_y_i) - y_i_1
    ...
    ### This is EDCIT for reference ###
    #denoising
    def forwardEDCIT(self, input_x):
        z_i, y_i = input_x[:1], input_x[1:]
        z_i_1 = self.p * z_i + (1-self.p) * y_i
        y_i_1 = self.p * y_i + (1-self.p) * z_i_1
        return torch.cat([z_i_1, y_i_1])
    
    #noising
    def inverseEDCIT(self, input_x):
        z_i_1, y_i_1 = input_x.split(1)
        y_i = (y_i_1 - (1-self.p) * z_i_1) / self.p
        z_i = (z_i_1 - (1-self.p) * y_i) / self.p
        return torch.cat([z_i, y_i])
    
class MixingLayerEDCIT(nn.Module):
    """
            ...
            with autocast(device):
                ...
                for i, t in tqdm(enumerate(timesteps.flip(0)), total=len(timesteps)):
                    ...
                    t = torch.tensor([t],
                                     dtype=torch_dtype,
                                     device=latent_pair.device)
                    #TODO# Implement reverse BDIA-DOODL:
                        #-need to get to BDIA inverse mixing equation working
            #previous implemenation
                    #latent_pair = mix.inverse(latent_pair, epsilons, alphas, sigmas)
                    #i, t, latent_pair = s.inverse(i, t, latent_pair)
                
                #BDIA implementation
                    if first:
                        _, _, _, epsilons, alphas, sigmas = s.inverse(i, t, latent_pair)
                        first = False
                    latent_pair = mix.inverse(latent_pair, epsilons, alphas, sigmas)
                    i, t, latent_pair, epsilons, alphas, sigmas = s.inverse(i, t, latent_pair)
            ...
            with autocast(device):
                for i, t in tqdm(enumerate(initial_timesteps),
                    ...
                    t = torch.tensor([t],
                                     dtype=torch_dtype,
                                     device=latent_pair.device)
                    i, t, latent_pair, epsilons, alphas, sigmas = s(i, t, latent_pair)
                    latent_pair = mix(latent_pair, epsilons, alphas, sigmas)
            ...
            for m in range(num_traversal_steps): # This is # of optimization steps
                ...
                with autocast(device):
                    for i, t in tqdm(enumerate(timesteps), total=len(timesteps)):
                        ...
                        #edited here for BDIA to allow for the extra variables to pass through
                        i, t, input_latent_pair, epsilons, alphas, sigmas = s(i, t, input_latent_pair)
                        input_latent_pair = mix(input_latent_pair, epsilons, alphas, sigmas)
                ...
                for im_i,im in enumerate(ims): 
                    ...
                    if grad_scale!=0:
                        loss = loss_fn(im)
                        losses.append(loss)
                        if optimize_first_edict_image_only: breaks
                sum_loss = sum(losses)
            ...
            with autocast(device):
                ...
                for i, t in tqdm(enumerate(timesteps.flip(0)), total=len(timesteps)):
                    ...
                    t = torch.tensor([t],
                                     dtype=torch_dtype,
                                     device=latent_pair.device)
                    # latent_pair = mix.inverse(i, t, latent_pair)
                    i, t, latent_pair, _, _, _ = s.inverse(i, t, latent_pair)
                    # print(latent_pair)
